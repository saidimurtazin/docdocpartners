...
bot.action('payout_se_no', async (ctx) => {
  ...
  await ctx.reply(
    '✅ Понятно!\n\n' +
    '💡 Рекомендуем оформить самозанятость для получения повышенного вознаграждения (10% вместо 7%).\n\n' +
    '3️⃣ Как вы хотите <b>получать выплаты</b>?',
    {
      parse_mode: 'HTML',
      ...Markup.inlineKeyboard([
...
bot.action('payout_confirm_request', async (ctx) => {
  ...
  try {
    ...
    message += '3️⃣ Для физлиц: подпишите акт через Контур.Сайн\n';
...
bot.action(/^kb_/, async (ctx) => {
  ...
  const answers: Record<string, string> = {
    ...
    kb_documents:
      ...
      'Выплаты и подписание документов осуществляются через нашего партнёра <b>Jump.Finance</b> с использованием сервиса электронной подписи <b>Контур.Сайн</b> (юридическая сила согласно 63-ФЗ).\n\n' +
      '<b>📋 Договор оферты:</b>\n' +
      'При регистрации вы соглашаетесь с договором оферты, который размещён по ссылке: <a href="https://docdocpartners-production.up.railway.app/oferta">docdocpartners.ru/oferta</a>\n\n' +
      '<b>💰 При выводе средств:</b>\n\n' +
      '👤 <b>Физическое лицо:</b>\n' +
      '• Вы подписываете Акт оказанных услуг через Контур.Сайн\n' +
      '• После подписания получаете выплату\n\n' +
      '📱 <b>Самозанятый:</b>\n' +
      '• Вы получаете поручение на оказание услуг с суммой выплаты\n' +
    ...
    kb_selfemployed:
      ...
      '<b>Сравнение:</b>\n' +
      '💚 Самозанятый: 7% от стоимости лечения (налог 6% от выплаты платите сами)\n' +
      '💛 Не самозанятый: ~4% (уже за вычетом НДФЛ 13% и соц. отчислений 30%)\n' +
